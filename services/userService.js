...
class UserService {
    ...
    updateApartment(data){
      return http.post('/api/apartments/update_apartment', data)
    }
    ...
    updateApproved(data){
      return http.put('/api/schedules/approve_completion', data)
    }
    // Profile 
    updateProfile(data) {
    ...
    deleteBeforePhoto(scheduleId, taskTitle, imageUrl) {
      const encodedImageUrl = encodeURIComponent(imageUrl);  // ✅ Proper encoding
  ...
  deleteSpaceAfterPhoto(data){
    ...
    createLinkUrl(accountId){
      return http.get(`/api/stripes/onboard_cleaner/${accountId}`)
    }
    getStatus(accountId){
    ...
    sendChatMessagePushNotification(data){
      return http.post('/api/push_notification/chat_messages', data)
    }
